...
pal_inv <- colorBin(palette = "RdYlBu", domain=inv_2010$WHPT_TOTAL_Mean)
...
addPolylines(data = DRN, 
  ...
  addCircleMarkers(data = inv_2010,
                   ...
                   lat = ~lat, 
                   radius = 5,
                   fillColor = ~pal_inv(inv_2010$WHPT_TOTAL_Mean),
                   color = "black",
                   weight = 0.3,
                   fillOpacity = 1,
                   popup = paste0("WHPT_TOTAL_Mean Score: ",inv_2010$WHPT_TOTAL_Mean,                        #Bit random dates? Check mean is calculated correctly. Also isn't it WHPT we should show not BMWP? I think the columns may be messed up as ANALAYSIS_ID is our date.
                                  "<br> Sample Range: ", range(inv_2010$Sample_Date_Maybe)),
                   group = "Biosys Invertebrate Sites"
                   )%>% 
  addLayersControl(baseGroups = c("Esri Basemap","Blank Basemap"),
                   overlayGroups = "Biosys Invertebrate Sites",
  ...
  hideGroup(c("Biosys Invertebrate Sites",
              "Esri Basemap")) %>% 
  addLegend(title= paste0("Mean WHPT Tot Site <br> Scores 2010 - 2018"),
            pal = pal_inv, values = inv_2019$WHPT_TOTAL_Mean)
...
ggplot(api, aes(DateTime, total))+geom_point(col="seagreen2")+geom_smooth(identity= "stat")+
  theme_dark()
