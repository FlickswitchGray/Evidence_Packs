...
The `r unique(CAT$OPCAT_NAME)` Operational Catchment sits within the `r unique(CAT$MNCAT_NAME)` Management Catchment, which sits within the larger `r unique(CAT$RBD_NAME)` River Basin District.

It contains `r length(unique(CAT$WB_NAME))` individual EA waterbodies.
...
DRN_OPO <- DRN %>% filter(OPCAT_NAME == unique(CAT$OPCAT_NAME))
