...
CDE_e_2015 <- CDE %>% 
 ...
 leaflet() %>% 
  ...
  addPolygons(data= CDE_e_2015,
              ...
              popup= ~Water.Body,
              group = "2015 Status")   %>%        
   addLayersControl(baseGroups = c("Esri Basemap",
                                   "Blank Basemap"),
                   overlayGroups = c(
              "2022 Status",
              "2019 Status",
...
#  addLegend(opacity = 0.9, 
 #           pal = pal,
  #          values = CDE_e_2019$Status,
   #         title = paste0("2019 <br>",unique(CDE_e_2019$Classification.Item),
    #                       " WFD Status")) 
# 2019 map


#map_2015 <- leaflet(CDE_e_2015) %>% 
 # addProviderTiles(providers$Esri) %>% 
  #addPolygons(color = "black",
   #           weight = 0.5,
    #          fillOpacity = 0.7,
...
#  addLegend(opacity = 0.9, 
 ###           pal = pal,
    #        values = CDE_e_2015$Status,
     #       title = paste0(unique(CDE_e_2015$Classification.Item),
...
CDE_f <- CDE %>% 
  filter(Classification.Item == "Invertebrates" & Year == "2022")
...
leaflet() %>% 
  addProviderTiles(providers$Esri) %>% 
   addPolygons(data=CDE_f,
                        ...
                        weight= 0.5,
                        fillOpacity = 0.99,
                        fillColor = ~pal(CDE_f$CLASSIFICATION_NAME),
              ...
              #          opacity = 0.99,
               #         fillColor = NA
                ...
                addLegend(opacity = 0.9, 
                         ...
                         title = paste0("2022 <br>",unique(CDE_f$CLASSIFICATION_NAME),
                           " WFD Status & RNAGs")) 
```
...
CDE_2022 <- leaflet(CDE_e) %>% 
  ...
  addLegend(opacity = 0.9, 
            pal = pal,
            values = CDE_e$Status,
            title = paste0("2022 <br>",unique(CDE_e$Classification.Item),
                           " WFD Status")) 
...
CDE_2019 <- leaflet(CDE_e) %>% 
  ...
  addLegend(opacity = 0.9, 
            pal = pal,
            values = CDE_e$Status,
             title = paste0("2019 <br>",unique(CDE_e$Classification.Item),
                           " WFD Status")) 
...
The below timeseries show WHPT_Total and WHPT_ASPT for all sites within `r unique(CAT$OPCAT_NAME)` catchment. WHPT Total is the sum of individual taxa scores. It reflects the overall biological quality of the water based on the presence and abundance of various taxa.
