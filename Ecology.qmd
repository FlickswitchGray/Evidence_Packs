...
ARMI <- read.csv("/dbfs/FileStore/WSX_HGray/ETL_Imports_Require_Manual/Riverfly_Manual_Export_021024.csv")
...
ARMI %<>%
  ...
  mutate(
      Survey_Count = n(),
      Mean_Tot = mean(ARMI.Total, na.rm = TRUE),
  ) %>% 
    ungroup()
...
addPolylines(data = Rivs250, 
  ...
  addCircleMarkers(data = ARMI,
                   ...
                   weight = 0.3,
                   fillOpacity = 1,
                   popup = paste0("Mean Total Score: ", ARMI$Mean_Tot,                        #Bit random dates? Check mean is calculated correctly. Also isn't it WHPT we should show not BMWP? I think the columns may be messed up as ANALAYSIS_ID is our date.
                                  "<br> Sample Date: ", range(ARMI$DateTime)),
