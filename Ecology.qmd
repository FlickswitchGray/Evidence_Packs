...
fish <- readxl::read_excel("/dbfs/FileStore/WSX_HGray/RiverFish_SiteData_Results_For_QA.xlsx") %>% 
        ...
        mutate(`Survey(Ranked)NGR` =  gsub("n/a","", `Survey(Ranked)NGR`, fixed=T)) #remove any junk from NGR grid reference
...
Fish_WGS84 <- fish %>% st_as_sf(coords= c("East", "North"), crs=27700) %>% 
                        st_transform(4326)

# Map the Site level Fish data as a toggle for 2022.
...
leaflet() %>% 
  addProviderTiles(providers$Esri,
                   group = "Esri Basemap") %>% 
  addPolygons() %>% 
  addCircles(data=Fish_WGS84,
             color = ~pal(Fish_WGS84$Description) # This is our classification
             )
