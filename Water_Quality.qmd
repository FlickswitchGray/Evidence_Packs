...
# Create map with ALERT data
  ...
  pal_ALERT_N<- colorNumeric(palette = "YlOrRd",  domain = ALERT_WB$Catch_Overall_Risk)
        ...
        leaflet() %>% 
          ...
          addPolygons(data = ALERT_WB,
                      ...
                      fillOpacity = 0.5,
                      color = 1,
                      popup = paste0("<b>Diffuse Pollution Risk: </b>", ALERT_WB$Catch_Overall_Risk,
                                     "<br><b>Waterbodies: </b>", ALERT_WB$WB_NAME),
                      group = "Catchment Risk") %>% 
          addPolylines(data = ALERT,
               fill = TRUE,
               fillColor = ~pal_ALERT(CatchmentRiskScore),
               fillOpacity = 1,
               opacity = 1,
               weight = 1,
               highlightOptions = weight = 3
          ...
          addLayersControl(overlayGroups = c("ALERT_WB"),
                           position = "topright",
                           options = layersControlOptions(collapsed = FALSE)) %>% 
          hideGroup(c("Diffuse Pollution Pathways"))
