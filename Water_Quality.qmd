...
sequ <- seq(min(sewage$X2022.spill.count), max(sewage$X2022.spill.count), length.out = 5)
    ...
    leaflet() %>%
      ...
      addCircleMarkers(
                      ...
                      label = paste0("Mean OrthoP: ",~result),
...
This initial evidence pack doesn't include any water quality Water Company data, only citizen science data taken directly from a national water quality monitoring scheme ran by the Angling Trust. Third-parties often have a different or un-aligned monitoring agenda to the Environment Agency. Eventually through influence, priorities can be aligned however, initially data may not be exactly where the EA would require it.
...
# Enter an if statement in case we don't have any data in our catchment   
  unique_count <- length(unique(df$Site_Name))
      ...
      #Plot mean nitrate on a map, experimental so only covers an arbitrary timespan
          ...
          print(paste0("The ", unique(CAT$OPCAT_NAME), " does not have any monitoring data collected in the Angling Trust's Water Quality Monitoring Network scheme to display"))
        }
