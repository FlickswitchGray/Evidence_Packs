...
The below map shows data from the Agricultural Land Environmental Risk Tool ([ALERT](https://defra.sharepoint.com/:u:/r/teams/Team646/SitePages/A.L.E.R.T-Home-Page.aspx?csf=1&web=1&e=hPErKt)), it models diffuse pollution pathways and quantifies diffuse pollution risk based on: 
- Land-use 
- Hill slope 
- Erosion and runoff risk of soil 
- Mean precipitation

The below map provides a mean score of diffuse pollution per waterbody based on the data from ALERT, there is also the option of viewing diffuse pollution pathways at a reach scale.

```{r Map from ALERT tool}

# The input file geodatabase
gdb <- st_read("/dbfs/FileStore/WSX_HGray/ALERT_WSX.gdb")


# Create a wb wide score
ALERT <- gdb %>%
          group_by(WATERBODY_NAME) %>%
          mutate(Catch_Overall_Risk = round(mean(CatchmentRiskScore),1) %>% 
          ungroup()

ALERT <- ALERT[CAT_27700,]
ALERT <-  ALERT %>% st_transform(4326)

ALERT_Risk <- ALERT[,c(2,23)]

colnames(ALERT_Risk)[1] <- "WB_NAME"

ALERT_WB <- ALERT_Risk %>% st_drop_geometry() %>%  
                           left_join(CAT, by = "WB_NAME") %>% 
                           st_as_sf()

pal_ALERT <- colorNumeric(palette = "YlOrRd", domain = ALERT_WB$Catch_Overall_Risk)
SEQ <- seq(min(ALERT$Catch_Overall_Risk), max(ALERT$Catch_Overall_Risk), length.out = 5)
   ...
   #         pal = pal_ALERT, 
    ##        values = SEQ
      #      title = "Diffuse Pollution Risk",
       #     group = "ALERT_WB") %>%   # Adjusted group to match the layer group
  addScaleBar() %>% 
  addLayersControl(overlayGroups = c("ALERT_WB"),
                   position = "topright",
                   options = layersControlOptions(collapsed = FALSE)) %>% 
  hideGroup(c("ALERT_WB"))
...
pal_am <- colorBin(palette = "inferno", domain = WIMS_am$result, bins = binsss, reverse=TRUE )
