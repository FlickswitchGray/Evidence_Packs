...
RNAG <- RFF %>% 
               ...
               filter(CLASS_ITEM_NAME %in% c("Hydrological Regime"))%>% 
...
Mes <- inner_join(Measures_WBs, Measures_Cat, by = "ACTION_UID") %>%
                filter(OPERATIONAL_CATCHMENT %in% c("Poole Harbour Rivers","Poole Harbour Rivers TraC")) %>% 
                inner_join(CAT_Mini, by = c("WATERBODY_ID" = "WB_ID")) %>% 
        arrange(WATERBODY_ID)

Mes <- Mes %>% filter(WATERBODY_ID == unique(RNAG$WATERBODY_ID))


   DT::datatable(Mes[,c(3, 16,6,12:13, 10, 15, 2)],
          class = 'cell-border stripe',
          extensions = c("Buttons","FixedColumns"),
          options = list(
            dom = 'Blfrtip',
